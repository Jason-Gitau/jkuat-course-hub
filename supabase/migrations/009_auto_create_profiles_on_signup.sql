-- Migration: Automatic Profile Creation on User Signup
-- Description: Creates a database trigger to automatically create a profile entry
--              whenever a new user signs up via auth, preventing users from existing
--              in auth.users without corresponding profiles entry.
--
-- NOTE: This migration must be applied via Supabase Dashboard (SQL Editor) or
--       Supabase CLI with elevated permissions, as it creates a trigger on auth.users
...
COMMENT ON TRIGGER on_auth_user_created ON auth.users IS 'Ensures every authenticated user has a corresponding profile entry';
