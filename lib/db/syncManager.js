...
export async function syncCourses() {
  try {
    ...
    // Fetch all courses (removed expensive material count query)
    ...
    // Add sync timestamp (materialsCount removed for performance - fetch when needed)
    const coursesWithSync =
      courses?.map((course) => ({
        ...course,
        _syncedAt: Date.now(),
      })) || [];
    ...
    // Store in IndexedDB
    await putManyInStore(STORES.COURSES, coursesWithSync);
    await updateLastSyncTime(STORES.COURSES);

    return { success: true, data: coursesWithSync, error: null };
  } catch (error) {
