 ...
 * - New uploads: Supabase (fast, integrated)
 * - Old/rarely accessed: R2 (cheap, large capacity)
 * - Hot materials: Keep in Supabase
 * - Cold materials: Migrate to R2
 *
 * Strategy:
 * - Materials < 60 days old: Supabase
 * - Materials > 60 days old with < 5 downloads: R2
 * - Popular materials (> 20 downloads): Always Supabase
 */
...
export async function uploadFile(fileBuffer, metadata) {
  ...
  try {
    ...
    // Default to Supabase for new uploads
    // (Migration to R2 happens later via background job)
